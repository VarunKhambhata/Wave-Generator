...
class wave extends JPanel implements ChangeListener,Runnable
{
    Thread t;
    JFrame f,ctr;
    ...
    public void paint(Graphics g) 
    {
        ...
        int St_x=wndw_x/2;
        int St_y=wndw_y/2;
       // g.fillOval(St_x,St_y,30,30);

        int off_x=-wndw_x/2;
        //System.out.println(off_x);
        int Lst_x=0,Lst_y=0;
             Lst_x=St_x+off_x;
            Lst_y=St_y+(int)Math.round( intensty* Math.sin(off_x +a));

        //System.out.print((int)Math.round( Math.sin(off_x + a)*100));       
